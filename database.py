...
class Database:
    ...
    def _init_files(self):
        ...
        if not os.path.exists(self.users_file):
            cols = ['user_id', 'role', 'first_name', 'last_name', 'email', 'password', 
        ...
        if not os.path.exists(self.orders_file):
            cols = ['order_id', 'customer_id', 'order_date', 'delivery_date', 
                    'status', 'total_amount', 'discount_amount', 'payment_method', 'discount_code']
        ...
        if not os.path.exists(self.reviews_file):
            cols = ['review_id', 'customer_id', 'order_id', 'rating', 'comment', 'review_date']
            pd.DataFrame(columns=cols).to_csv(self.reviews_file, index=False)

        # Discount Codes (Phase 5)
        if not os.path.exists(self.discount_codes_file):
            cols = ['code', 'discount_percentage', 'expiry_date', 'is_used', 'customer_id']
            pd.DataFrame(columns=cols).to_csv(self.discount_codes_file, index=False)

    # --- Users ---
    def load_users(self) -> pd.DataFrame: return pd.read_csv(self.users_file)
    ...
    def save_user(self, user: User):
        ...
        user_data = {
            'user_id': user.user_id, 'role': user.get_role(),
        ...
        pd.concat([df, pd.DataFrame([user_data])], ignore_index=True).to_csv(self.users_file, index=False)
    ...
    def update_user_profile(self, email: str, updated_fields: dict):
        ...
        if len(index) > 0:
            for field, value in updated_fields.items():
                if field in df.columns: df.at[index[0], field] = value
            df.to_csv(self.users_file, index=False)
    ...
    def _parse_dates(self, date_str: str) -> List[date]:
        ...
        try: dates = json.loads(date_str); return [datetime.strptime(d, "%Y-%m-%d").date() for d in dates]
        except: return []
    ...
    def save_food(self, food: Food):
        df = self.load_foods()
        food_data = {
            'food_id': str(food.food_id), 'name': str(food.name), 'category': str(food.category),
            'selling_price': float(food.selling_price), 'cost_price': float(food.cost_price),
            'ingredients': str(food.ingredients), 'description': str(food.description),
            'stock': int(food.stock), 'available_dates': self._format_dates(food.available_dates),
            'image_path': str(food.image_path) if food.image_path else None
        }
        cols = ['food_id', 'name', 'category', 'selling_price', 'cost_price', 
                'ingredients', 'description', 'stock', 'available_dates', 'image_path']
        pd.concat([df, pd.DataFrame([food_data], columns=cols)], ignore_index=True).to_csv(self.foods_file, index=False)

    # --- Orders ---
    def save_order(self, order):
        if os.path.exists(self.orders_file) and os.path.getsize(self.orders_file) > 0: df = pd.read_csv(self.orders_file)
        else: df = pd.DataFrame(columns=['order_id', 'customer_id', 'order_date', 'delivery_date', 'status', 'total_amount', 'discount_amount', 'payment_method', 'discount_code'])
        
        order_data = {
            'order_id': order.order_id, 'customer_id': order.customer_id,
            'order_date': order.order_date.strftime("%Y-%m-%d %H:%M:%S"),
            'delivery_date': order.delivery_date.strftime("%Y-%m-%d"),
            'status': order.status, 'total_amount': order._total_amount,
            'discount_amount': order.discount_amount, 'payment_method': order.payment_method,
            'discount_code': order.discount_code
        }
        pd.concat([df, pd.DataFrame([order_data])], ignore_index=True).to_csv(self.orders_file, index=False)

    def save_order_items(self, order_id: str, items):
        if os.path.exists(self.order_items_file) and os.path.getsize(self.order_items_file) > 0: df = pd.read_csv(self.order_items_file)
        else: df = pd.DataFrame(columns=['order_id', 'food_id', 'quantity', 'unit_price'])
        
        items_data = [{'order_id': order_id, 'food_id': i.food.food_id, 'quantity': i.quantity, 'unit_price': i.unit_price} for i in items]
        if items_data: pd.concat([df, pd.DataFrame(items_data)], ignore_index=True).to_csv(self.order_items_file, index=False)
